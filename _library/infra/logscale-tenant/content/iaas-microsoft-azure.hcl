...
inputs = {
  ...
  release          = join("-", compact(["logscale", local.prefix, local.name, local.codename, "content-azure", local.suffix]))
...
repositories:
  - name: ${join("-", compact([local.prefix, "iaas-azure", local.suffix]))}
    ingestTokens:
      - name: function
  - name: ${join("-", compact([local.prefix, "iaas-azuread", local.suffix]))}
    ingestTokens:
      - name: function
