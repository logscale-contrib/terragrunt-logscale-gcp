...
inputs = {
  ...
  release          = join("-", compact(["logscale", local.prefix, local.name, local.codename, "content-windows", local.suffix]))
...
repositories:
  - name: ${join("-", compact([local.prefix, "infra-os-windows-security", local.suffix]))}
    ingestTokens:
      - name: windows
        parserName: 
  - name: ${join("-", compact([local.prefix, "infra-os-windows-members", local.suffix]))}
    ingestTokens:
      - name: windows
        parserName: 
  - name: ${join("-", compact([local.prefix, "infra-os-windows-dcs", local.suffix]))}
    ingestTokens:
      - name: windows
        parserName: 
