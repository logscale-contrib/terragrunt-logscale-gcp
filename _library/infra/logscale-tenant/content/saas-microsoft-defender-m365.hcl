...
inputs = {
  ...
  release          = join("-", compact(["logscale", local.prefix, local.name, local.codename, "content-m365-defender", local.suffix]))
...
repositories:
  - name: ${join("-", compact([local.prefix, "saas-m365-defender", local.suffix]))}
    ingestTokens:
      - name: function
EOF
  )
